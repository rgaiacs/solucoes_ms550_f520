...
% Last Change: 14.07.12 10:18:36 AM
% 
% Authors:
...
\begin{questions}
    ...
    \begin{solution}
        ...
        Para a segunda solução, visto que $r_1 - r_2 = 1 \in \mathbb{N}$, temos
        
        \begin{align*}
        		y_2 = k y_1 \ln x + \sum^\infty_{n=0}b_n x^{n+r_2}
        \end{align*}
        
        Substituindo na equação original e efetuando as manipulações necessárias chegamos em:
        
        \begin{align*}
        		k \left[-x^{-\frac{1}{2}} \sin{x} + 2 x^{\frac{1}{2}} \cos{x}\right] + \sum^\infty_{n=2}\left[n(n-1)b_n + b_{n-2}\right]x^{n-\frac{1}{2}} = 0
        \end{align*}
        
        Com $b_0$ e $b_1$ livres. O lado esquerdo da expressão implica que $k = 0$. Pelo lado direito chegamos na seguinte relação de recorrência:
        
        \begin{align*}
			b_n = -\frac{b_{n-2}}{n(n-1)}, n \ge 2        
        \end{align*}
        ...
        \begin{align*}
			b_2 &= -\frac{b_0}{2.1}\\
			b_3 &= -\frac{b_1}{3.2}\\
			b_4 &= -\frac{b_2}{4.3} = \frac{b_0}{4.3.2.1}\\
			b_5 &= -\frac{b_3}{5.4} = \frac{b_1}{5.4.3.2}			        
        \end{align*}
        
        Tomando $b_0 =1$ e $b_1 = 0$ chegamos na seguinte solução da relação de recorrência:
        \begin{align*}
			b_n = \frac{\left(-1\right)^n}{\left(2n\right)!}, n \ge 0       
        \end{align*}
        
        portanto,
        
        \begin{align*}
        		y_2 &= \sum^\infty_{n=0}\frac{\left(-1\right)^n}{\left(2n\right)!} x^{2n-\frac{1}{2}} \\
        			&= x^{-\frac{1}{2}}\cos{x}
        \end{align*}
    ...
    % Resposta
    % y_1 = \sum_{k = 0}^\infty (w x)^{-2 k} / (2k)!
    % y_2 = \sum_{k = 0}^\infty (w x)^{-2k - 1} / (2k + 1)!
    \begin{solution}
    ...
    \question $y'' + x y' + y = 0$.
    % Resposta
    % a_0 e a_1 arbitr\'{a}rios, a_k = - \frac{1}{k} a_{k - 2}, k \geq 2
    \begin{solution}
    ...
    % Resposta
    % a_0 e a_1 arbitr\'{a}rios, a_2 = a_3 = a_4 = 0 e a_k = - \frac{5}{k(k - 1)} a_{k - 5)}, k \geq 5
    \begin{solution}
    ...
    % Resposta
    % a_k = \frac{1}{2k} a_{k - 1}, k \geq 1
    % a_k = \frac{1}{2k + 1} a_{k - 1}, k \geq 1
    \begin{solution}
    ...
    % Resposta
    % a_1 = a_2 = 0
    % a_k = - \frac{1}{k(k + 2 \sqrt{2}} a_{k - 3}, k \geq 3
    % a_k = - \frac{1}{k(k - 2 \sqrt{2}} a_{k - 3}, k \geq 3
    \begin{solution}
        Manipulando a equa\c{c}\~{a}o temos
        \[
    ...
    % Resposta
    % a_{k + 1} = - \frac{(3k - 2)(3k + 2)}{(3k + 3)(3k + 4)} a_k, k \geq 1
    % y_2 = 1 + x / 2
    \begin{solution}
        Manipulando a equa\c{c}\~{a}o temos
        \[
    ...
    % Resposta
    % a_k = \frac{1}{k (2k - 1)} a_{k - 1}, k \geq 1
    % a_k = \frac{1}{k (2k _ 1)} a_{k - 1}, k \geq 1
    \begin{solution}
        Manipulando a equa\c{c}\~{a}o temos
        \[
    ...
    % Resposta
    % a_k = a_{k - 1} / \left[ 8 (k + r) (k + r - 1) + 2 (k + r) + 1 \right], k \geq 1
    % r_1 = -1/2
    % r_2 = -1/4
    \begin{solution}
        Manipulando a equa\c{c}\~{a}o temos
    ...
    \end{align*}
    \begin{parts}
        \part Encontre sua solu\c{c}\~{a}o geral na forma de s\'{e}rie em torno do ponto $x_0 = 0$.
        \begin{solution}
            % TODO Escrever solu\c{c}\~{a}o.
        \end{solution}

        \part Encontre sua solu\c{c}\~{a}o geral na forma de s\'{e}rie em torno do ponto $x_0 = 1$.
        \begin{solution}
            % TODO Escrever solu\c{c}\~{a}o.
        \end{solution}

        \part Compare as solu\c{c}\~{o}es obtidas nos itens anteriores.
        \begin{solution}
            % TODO Escrever solu\c{c}\~{a}o.
        \end{solution}
    \end{parts}
    ...
    \begin{solution}
        % TODO Escrever solu\c{c}\~{a}o.
        % y = x^{3/4} \sum_{k = 0}^\infty \frac{(-1)^k 2Ç k x^{3k}}{3^{2k} k! (2k + 1)!!}
        % y = x^{-3/4} \left[ 1 + \sum_{k = 1}^\infty \frac{(-1)^k 2^k x^{3k}}{3^{2k} k! (2k - 1)!!} \right]
    \end{solution}
    ...
    \begin{solution}
        % TODO Escrever solu\c{c}\~{a}o.
        % a_n = - a_{n - 1} / \left[ 2(n + k)^2 - 1 \right] = - a_{n - 1} / \left[ 2n (n \pm \sqrt{2} \right]
    \end{solution}
    ...
    \begin{parts}
        \part Encontre uma solu\c{c}\~{a}o n\~{a}o-trivial para a equa\c{c}\~{a}o diferencial.
        \begin{solution}
            % TODO Escrever solu\c{c}\~{a}o.
        \end{solution}

        \part Para quais valores de $\nu$ voc\^{e} pode garantir que a equa\c{c}\~{a}o diferencial tem uma solu\c{c}\~{a}o polinomial?
        \begin{solution}
            % TODO Escrever solu\c{c}\~{a}o.
        \end{solution}

        \part Encontre a solu\c{c}\~{a}o geral da equa\c{c}\~{a}o diferencial para $\nu = 1$ em termos de s\'{e}rie de pot\^{e}ncias generalizadas.
        \begin{solution}
            % TODO Escrever solu\c{c}\~{a}o.
        \end{solution}
    \end{parts}
    ...
    \end{align*}
    \begin{solution}
        % TODO Terminar de digitar.
        % TODO Melhorar a reda\c{c}\~{a}o.
    ...
    \question[T2 de 2012, P1 de 2012, E de 2012] Seja a equa\c{c}\~{a}o diferencial $x(1 - x) y'' + (1 - 5x) y' - 4y = 0$. Ao utilizar o m\'{e}todo de Frobenius para resolver essa equa\c{c}\~{a}o encontramos que a equa\c{c}\~{a}o indicial correspondete apresenta ra\'{i}zes reais iguais a $r_1 = r_2 = 0$, e com isso obtemos que uma das solu\c{c}\~{o}es em forma de s\'{e}rie dessa equa\c{c}\~{a}o \'{e}
    \begin{align*}
